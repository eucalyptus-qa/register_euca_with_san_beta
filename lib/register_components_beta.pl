...
sub register_ws{
	...
	for( my $i = 0; $i < $index; $i++){
		if( does_It_Have($roll_lst[$i], "WS") ){
			...
			#Register Walrus
			print("ssh -o StrictHostKeyChecking=no root\@$this_clc_ip \"source /root/eucarc; $ENV{'EUCALYPTUS'}/usr/sbin/euca_conf --register-walrus --partition WALRUS00 --component $comp_name --host $ws_ip\"\n");
			$outstr = `ssh -o StrictHostKeyChecking=no root\@$this_clc_ip \"source /root/eucarc; $ENV{'EUCALYPTUS'}/usr/sbin/euca_conf --register-walrus --partition WALRUS00 --component $comp_name --host $ws_ip\"`;
...
sub register_cc{
	...
	for( my $i = 0; $i < $index; $i++){
		...
		if( $roll_lst[$i] =~ /CC$group/ ){
			...
			#Register CC
			print("ssh -o StrictHostKeyChecking=no root\@$this_clc_ip \"source /root/eucarc; $ENV{'EUCALYPTUS'}/usr/sbin/euca_conf --register-cluster --partition PARTI$group --component $comp_name --host $my_cc_ip\"\n");
			$outstr = `ssh -o StrictHostKeyChecking=no root\@$this_clc_ip \"source /root/eucarc; $ENV{'EUCALYPTUS'}/usr/sbin/euca_conf --register-cluster --partition PARTI$group --component $comp_name --host $my_cc_ip\"`;
...
sub register_nc{
	...
	foreach my $my_nc_ip (@my_nc_ips){
		...
		#Register NCs		
		print("ssh -o StrictHostKeyChecking=no root\@$my_cc_ip \"source /root/eucarc; $ENV{'EUCALYPTUS'}/usr/sbin/euca_conf --register-nodes $my_nc_ip\"\n");
		$outstr = `ssh -o StrictHostKeyChecking=no root\@$my_cc_ip \"source /root/eucarc; $ENV{'EUCALYPTUS'}/usr/sbin/euca_conf --register-nodes $my_nc_ip\"`;
...
sub register_sc{
	...
	for( my $i = 0; $i < $index; $i++){
		if( $roll_lst[$i] =~ /SC$group/ ){
			...
			print("ssh -o StrictHostKeyChecking=no root\@$this_clc_ip \"source /root/eucarc; $ENV{'EUCALYPTUS'}/usr/sbin/euca_conf --register-sc --partition PARTI$group --component $comp_name --host $my_sc_ip\"\n");
			$outstr = `ssh -o StrictHostKeyChecking=no root\@$this_clc_ip \"source /root/eucarc; $ENV{'EUCALYPTUS'}/usr/sbin/euca_conf --register-sc --partition PARTI$group --component $comp_name --host $my_sc_ip\"`;
...
sub register_vmbroker{
	...
	for( my $i = 0; $i < $index; $i++){
		if( $roll_lst[$i] =~ /CC$group/ ){
			...
			print("ssh -o StrictHostKeyChecking=no root\@$this_clc_ip \"source /root/eucarc; $ENV{'EUCALYPTUS'}/usr/sbin/euca_conf --register-vmwarebroker test$group $my_vb_ip\"\n");
			$outstr = `ssh -o StrictHostKeyChecking=no root\@$this_clc_ip \"source /root/eucarc; $ENV{'EUCALYPTUS'}/usr/sbin/euca_conf --register-vmwarebroker test$group $my_vb_ip\"`;
