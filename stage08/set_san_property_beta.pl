...
if( $san_provider eq "NetappProvider" ){
	...
	print "$clc_ip :: source /root/eucarc; $ENV{'EUCALYPTUS'}/usr/sbin/euca-modify-property -p test00.storage.sanhost=192.168.7.189\n";
	system("ssh -o StrictHostKeyChecking=no root\@$clc_ip \"source /root/eucarc; $ENV{'EUCALYPTUS'}/usr/sbin/euca-modify-property -p test00.storage.sanhost=192.168.7.189\" ");
	sleep(1);


	print "$clc_ip :: source /root/eucarc; $ENV{'EUCALYPTUS'}/usr/sbin/euca-modify-property -p test00.storage.sanuser=grpadmin\n";
	system("ssh -o StrictHostKeyChecking=no root\@$clc_ip \"source /root/eucarc; $ENV{'EUCALYPTUS'}/usr/sbin/euca-modify-property -p test00.storage.sanuser=grpadmin\" ");
	sleep(1);
